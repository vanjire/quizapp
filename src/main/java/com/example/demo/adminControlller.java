...
public String startQuiz(long qid,ModelMap mp,HttpSession session) {
...
public String getAnswer(@RequestParam("questionID") String[] questionID,@RequestParam("quizeID") String quizeID,HttpServletRequest request,HttpSession session,HttpServletResponse resp ) {
...
public String result(@RequestParam(defaultValue = "0") int page,long qid,HttpSession session,ModelMap mp) {
	if((String)session.getAttribute("uemail")==null){return "redirect:/uLogin";}
...
public String viewde(long qid,HttpSession session,ModelMap mp) {
	if((String)session.getAttribute("uemail")==null){return "redirect:/uLogin";}
	String em=(String) session.getAttribute("uemail");
...
public String login() {
	...
	public String loginEmail(String name,String email,RedirectAttributes ra) {
	...
	public String verify(String vcode,String email,String name,HttpSession session,RedirectAttributes ra) {
		
		try{if(!sr.isNumber(vcode)) {
			 ra.addFlashAttribute("email", email);
			 ra.addFlashAttribute("name", name);
			 ra.addFlashAttribute("msg", "Invalid code");
			return "redirect:/uLogin";
		}
	...
	public String Home(ModelMap mp,HttpSession session) {
		//if((String)session.getAttribute("uemail")==null){return "redirect:/uLogin";}
	...
	@GetMapping("/toStarts")
	public String toStart(ModelMap mp,HttpSession session) {
		if((String)session.getAttribute("uemail")==null){return "redirect:/uLogin";}
	...
	public String dashboard( Model model,ModelMap mp,HttpSession session) {
